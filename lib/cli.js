...
if (args.length === 1) {
  if (args[0] === '--help') {
    ...
    (0, _mdLinks.mdLinks)(path, options).then(resp => resp.forEach(values => console.log(` Path: ${values.file}\n Link: ${values.href}\n Text: ${values.text}\n`))).catch(err => console.log(err));
  }
}
...
if (args.length === 2) {
  if (args[1] === '--validate' || args[1] === '--v') {
    ...
    (0, _mdLinks.mdLinks)(path, options).then(resp => resp.map(values => console.log(` Path: ${values.file}\n Link: ${values.href}\n Status: ${values.status}\n StatusText: ${values.message}\n Text: ${values.text}\n`))).catch(err => err);
  } else if (args[1] === '--stats' || args[1] === '--s') {
    Promise.all([totalLinksStats(path), uniqueLinksStats(path)]).then(resp => resp.forEach(values => console.log(values))).catch(err => console.log(err));
  }
}
...
if (args.length === 3) {
  if ((args[1] === '--validate' || args[1] === '--v') && (args[2] === '--stats' || args[2] === '--s')) {
    Promise.all([totalLinksStats(path), uniqueLinksStats(path), brokenLinksStats(path)]).then(resp => resp.forEach(values => console.log(values))).catch(err => console.log(err));
